...
static enum hrtimer_restart hrtimer_event(struct hrtimer *timer)
{
	...
	do {
		 ...
		 */
		if (unlikely(i >= failure_iterations &&
			ts1.tv_sec  == ts2.tv_sec &&
			ts1.tv_nsec == ts2.tv_nsec)) {
				pr_err("pps_gen_parport: clock stay unchanged for too long.\n");
				goto error;
		}
	...
	do {
		 ...
		 * and prevent hard lockups.
		 */
		if (unlikely(i > gettime_attempts)) {
			pr_err("pps_gen_parport: gettime attempts threshold is exceeded\n.");
			goto error;
		}
